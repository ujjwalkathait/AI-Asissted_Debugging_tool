...
document.addEventListener('DOMContentLoaded', function () {
    ...
    debugBtn.addEventListener('click', async function () {
        ...
        try {
            ...
            debugOutput.textContent = data.fixedCode || "// No code fix returned.";
            errorOutput.innerHTML = data.errors
                ? `<pre style="color:red;">${data.errors}</pre>`
                : `<pre style="color:lightgreen;">No syntax errors detected.</pre>`;
        } catch (err) {
            errorOutput.innerHTML = `<pre style="color:red;">Error: ${err.message}</pre>`;
        } finally {
            // Reset button
            debugBtn.innerHTML = '<i class="fas fa-bug"></i> Debug Code';
            debugBtn.disabled = false;
